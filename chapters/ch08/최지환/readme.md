...
> 사진에는 292~295쪽(에이전트 선택 의사결정 트리, 프론트매터 11필드)도 있으나 이번 정리 범위에서 제외. 필요 시 다음에 보강.
...
실제로 참조 스터디 저장소(ggplab/claude-agentic-study ch08)의 검증에 따르면, 책 기준 버전(MoAI-ADK v2.14.0) 코드베이스에서 **5개 기둥 중 실제 기계 수준 게이트키핑이 있던 것은 2개(LSP·정적 분석)뿐이고, TDD 강제 등은 미구현 스텁 핸들러**였다고 한다. "품질 게이트"라는 이름이 붙어 있어도 **문서가 약속한 강제와 코드가 구현한 강제는 다를 수 있다** — 4회 fix-log에서 겪은 "등록본과 동봉본의 동작 불일치"와 정확히 같은 교훈이다. 프레임워크를 평가할 때 물을 질문: *"이 기준을 어기면 무엇이 물리적으로 실패하는가?"*
...
- ggplab 스터디가 지적한 버전 불일치(책 v2.14.0 vs 저장소 v3.0.1) — v3.x에서 스텁이었던 TDD 게이트가 실제 구현됐는지 추적해 볼 것.
