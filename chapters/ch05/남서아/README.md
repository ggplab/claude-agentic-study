...
### 애초에 "I don't know" 허용은 왜 통하는가

1. LLM은 그럴듯한 이어짓기를 뽑는 기계라 **환각이 기본 동작**이고,
2. 훈련·벤치마크가 "모르겠다"(빈칸)보다 자신 있는 오답(찍기)에 점수를 줘서 **찍도록 최적화됐는데** — [OpenAI 2025, arXiv 2509.04664](https://arxiv.org/abs/2509.04664),
3. 정작 모델 내부엔 "내가 이걸 아는지"에 대한 **캘리브레이션 신호(P(IK))가 이미 있다** — [Anthropic 2022, arXiv 2207.05221](https://arxiv.org/abs/2207.05221).

→ "모른다고 해도 된다"는 지침은 2가 억누른 출력 경로를 열어 3이 겉으로 나오게 하는 것. 단, 모델이 자신 있게 틀리는 영역에선 안 통한다("mostly"의 한계) — 그래서 도구 검증 강제가 별도 축으로 필요하다.
