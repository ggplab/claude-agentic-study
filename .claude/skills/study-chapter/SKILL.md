...
**스타일 (필수)** — 모임에서 여러 명이 같은 화면을 보며 이야기하는 자료다:
- **개조식**: 불릿·표·짧은 항목 중심으로 쓴다. 문단형 산문 금지.
- **artifact 시각화**: 핵심 내용은 standalone HTML 도식으로 만들어 같은 폴더에 두고(Pages 인앱 뷰용), GitHub README에서 바로 보이도록 PNG로 렌더해 임베드한다(`![...](xxx.png)` + HTML 원본 링크).
